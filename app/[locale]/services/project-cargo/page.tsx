...
import { Globe, Clock, Shield, Award, Users, CheckCircle, Mail, Phone } from "lucide-react"
import Image from "next/image"
import Link from "next/link"
import { useParams } from "next/navigation"
import { useTranslations } from "next-intl"

export default function ServicesProjectCargoPage() {
    const params = useParams()
    const locale = params.locale as string
    const t = useTranslations("servicesProjectCargo")
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            ...
            <section className="relative h-72 md:h-96 flex items-center justify-center overflow-hidden">
                <Image src="/images/sproject.png" alt="Hero Project Cargo" fill className="object-cover" priority />
            ...
            <main className="container mx-auto px-4 py-12 md:py-20 grid md:grid-cols-3 gap-8">
                ...
                <article className="md:col-span-2 bg-white rounded-2xl shadow-md p-8">
                    ...
                    {/* Sections */}
                    {["definition", "features", "costFactors", "process", "tips"].map((section) => (
                        <div key={section} className="mb-8">
                            <h3 className="text-xl font-semibold mb-2">{t(`content.${section}.title`)}</h3>
                            <p className="text-gray-700">{t(`content.${section}.desc`)}</p>
                        </div>
                    ))}
                    <div className="mt-10">
                        <h3 className="text-xl font-semibold mb-2">{t("content.conclusion.title")}</h3>
                        <p className="text-gray-700">{t("content.conclusion.desc")}</p>
                        <br />
                        <p className="text-gray-700">{t(`content.conclusion.desc2`)}</p>
                    ...
                    <div className="mt-10 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
                        <h3 className="text-xl font-bold mb-3">{t("content.cta.title")}</h3>
                        <p className="text-gray-600 mb-4">{t("content.cta.desc")}</p>
                        <Link href={`/${locale}/contact`}>
                            <Button className="bg-blue-600 hover:bg-blue-700 text-white">{t("content.cta.button")}</Button>
                        </Link>
                    </div>
                </article>
